...
Student Chat is a conversational interview (`chat_turn`), Grok infers from free speech and
only asks what’s still missing. **Talk** uses Grok Voice (speech-to-speech, Eve). Field-by-field
rules are in **[docs/grok-interview.md](docs/grok-interview.md)**.

Other call sites:

1. `extract_profile()`: free text → structured profile (batch / organizer parse).
...
The encrypted xAI key in `backend/.env.shared` is decrypted at startup. If that fails, every
function falls back to a small heuristic so the app still runs.
