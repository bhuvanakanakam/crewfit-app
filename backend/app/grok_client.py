...
PROFILE_JSON_SCHEMA = {
    ...
    "schema": {
        ...
        "properties": {
            ...
            "availability": {
                "type": "array",
                "items": {"type": "string", "enum": ["weekday_morning", "weekday_afternoon", "weekday_evening", "weekend"]},
            },
            "skills": {
                "type": "object",
                "properties": {
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "numbers directly — describe what they mean."
                ),
            },
            {
                "role": "user",
...
def _heuristic_extract(bio: str) -> dict:
    ...
    if re.search(r"weekend", text):
        availability.append("weekend")
    if re.search(r"evening", text):
        availability.append("weekday_evening")
    if re.search(r"morning", text):
        availability.append("weekday_morning")
    if re.search(r"afternoon", text):
        availability.append("weekday_afternoon")
    if not availability:
        availability = ["weekday_evening"]
        questions.append("When are you generally free to meet — weekday mornings, afternoons, evenings, or weekends?")
